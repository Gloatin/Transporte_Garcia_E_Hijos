...
if($libre_v2!=2)		{$libre_v2=2;	
	class libre_v2	{		
		...
		function input2				($type2,$name,$title,$value,$style,$id,$libre,$class)										{
			...
			if($id<>''){$cid="id='$id'";}
			$d="<input type='$type2' 		style='$style' $cid class='$class' name='$name' value='$value' 	title='$title' $libre >";
			if($type2==label)$d="<label 	style='$style' $cid class='$class' name='$name' 				title='$title' $libre >$value</label>";
			if($type2==tarea)$d="<textarea 	style='$style' $cid class='$class' name='$name'					title='$title' $libre >$value</textarea>";
			return $d;
		}	
		...
		function consulta			($tabla,$conexion,$col_espe,$espe,$orde,$dire,$phpv,$buscar,$pre_sql)								{ 
			...
			if ($pre_sql<>"")	{echo$consulta;}
			$consu=libre_v2::ejecuta($conexion,$consulta,$phpv);
			return $consu;
		}
		...
		function despliegre_mysql	($name,$name2,$consu,$descarga,$phpv,$libre,$className,$dataset,$set)												{
			...
			if($name2<>'')$d=$d."<OPTION value='$name'>$name2</OPTION>";
				...
				while($datos= libre_v1::mysql_fe_ar($res,$phpv)){$set='';		
					...
					if($dataset<>""){if($datos[$dataset]==$set){$set='selected';}}
		...
		function menu				($type,$style,$libre,$name,$v1,$v2,$v3,$v4,$v5,$v6,$v7,$v8,$v9,$v10,$v11,$script_input)			{	
			...
			if($v1<>'')$conte=$conte.	libre_v1::input2($type,$name,'',$v1,"",$d1 ,''.$script_input,'');
			if($v2<>'')$conte=$conte.	libre_v1::input2($type,$name,'',$v2,"",$d2 ,''.$script_input,'');
			if($v3<>'')$conte=$conte.	libre_v1::input2($type,$name,'',$v3,"",$d3 ,''.$script_input,'');
			if($v4<>'')$conte=$conte.	libre_v1::input2($type,$name,'',$v4,"",$d4 ,''.$script_input,'');
			if($v5<>'')$conte=$conte.	libre_v1::input2($type,$name,'',$v5,"",$d5 ,''.$script_input,'');
			if($v6<>'')$conte=$conte.	libre_v1::input2($type,$name,'',$v6,"",$d6 ,''.$script_input,'');
			if($v7<>'')$conte=$conte.	libre_v1::input2($type,$name,'',$v7,"",$d7 ,''.$script_input,'');
			if($v8<>'')$conte=$conte.	libre_v1::input2($type,$name,'',$v8,"",$d8 ,''.$script_input,'');
			if($v9<>'')$conte=$conte.	libre_v1::input2($type,$name,'',$v9,"",$d9 ,''.$script_input,'');
			if($v10<>'')$conte=$conte.	libre_v1::input2($type,$name,'',$v10,"",$d10);
			if($v11<>'')$conte=$conte.	libre_v1::input2($type,$name,'',$v11,"",$d11);
	...
	class tablas_v2	{
		function info($db,$tb){//tablas_v2::info(empresa,folio);
			...
			if($db==$db1){
				...
				if($tb==$tb25){//choferes
					$traducion="auto";
					$array_mysql	= array(ID_Ch	,Nombre_Ch,Edad,Direccion, Celular,ulti_viaje,Estatus);
					$array_name 	= array(Clave	,Operador,Edad,Direccion,"Celular Asignado","Ultima Carta Porte");
